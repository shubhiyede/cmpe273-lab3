...
public class RendezvousHash<K, N extends Comparable<? super N>> {
	...
	public void insertData(String server,int key,String data){
		
		//System.out.println("Destination Server: "+server);
    	CacheServiceInterface cache = new DistributedCacheService(server);    
        cache.put(key, data);
        System.out.println("Server: "+server +" put("+key+" => "+data+")");

	}
	
	public void getData(String server,int key){
		
		//System.out.println("Destination Server: "+server);
		CacheServiceInterface cache = new DistributedCacheService(server);    
		String value = cache.get(key);
        System.out.println("Server: "+server +" get("+key+") => " + value);
	}
